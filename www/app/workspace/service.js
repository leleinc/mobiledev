...
.factory('AuthService', function($http,UrlService) {
...
}).factory("PushService", function($http,UrlService, LoginService, $state, $ionicLoading, $timeout) {
  var JPushService = {
    smPushService: null,
    init: function(smPushService){
      ...
      if (window.plugins && window.plugins.jPushPlugin){
        ...
        alert('Not install JPushService plugin!');
      }
    },
    onGetRegistradionID: function(data) {
      ...
      try{
        ...
        if (data){
          ...
          console.log("JPushService:registrationID is "+data);
          //alert("JPushService:registrationID is "+data);
          //window.prompt("JPushService:registrationID is:", data);
          //$("#registrationid").html(data);
        } else {
          alert('无法获取设备id, 需要退出应用, 重新连接一下wifi!');
        }
      }
      catch(exception){
    ...
    */
    onOpenNotification: function(event){
      ...
      //alert("onOpenNotification-smPushService.isLogined:" + smPushService.isLogined);
      ...
      try{
        ...
        //console.log(extras);
        //alert("JPushService-onOpenNotification:" + alertMsg);
  ...
  var pushNotificationService = {
    ...
    smPushService: null,
    init: function(smPushService){
      //alert('pushNotificationService initing...');
      var me = this;
      me.smPushService = smPushService;
      if (window.plugins && window.plugins.pushNotification){
        smPushService.pushPlugin =  window.plugins.pushNotification;
      
        //alert('me.smPushService.pushPlugin.register:' + me.smPushService.pushPlugin.register);
        //alert('window.plugins.pushNotification register...');
        ...
        //alert("ionic 没有安装pushNotification插件, 无法支持推送功能!");
        console.log("ionic 没有安装pushNotification插件, 无法支持推送功能!");
      }
    },
    tokenHandler: function(result) {
      var me = this;
      //me.smPushService.isReady = true;
      //me.smPushService.deviceId = result;
      //alert("pushNotification初始化成功, 设备编码:" + result);
      //window.prompt("pushNotification初始化成功, 设备编码:", result);
      me.smPushService.onReady(result);
    },
    errorHandler: function(error) {
      //alert("pushNotification初始化错误:" + error);
      console.log("pushNotification初始化错误:" + error);
    },
    onNotificationAPN: function(event) {
      ...
      if ( event.alert ){
        ...
        if (event.foreground === "1"){
          //alert("logined, foreground message(todo):" + event.alert);
			$ionicLoading.show({
          ...
          //alert("logined, background message(cache):" + event.alert);
          smPushService.gotoUrl(event);
          console.log("已登录, 后台消息:" + event.alert);
          //window.eventGotoUrl = "page from push alert!";
          //alert("go to page!");
          //$state.go("tab.home.todo");
          
          //
          //window.eventGotoUrl = alertJson.gotoUrl;
          //var alertJson = JSON.parse(event.alert);
          //String 
          //alert("跳转URL:" + alertJson.gotoUrl);
        }
      }
    }     
  };
  var PushService = {
    ...
    isLogined: false,//AuthService.isAuthed这个属性不好用, 改用这个开控制
    eventsCache: [],
    /*测试代码
    ...
    */
    ...
    bindReceiveEvent: function(){
      ...
      if (ionic.Platform.isIOS()){
        ...
        console.log("推送不支持的操作系统平台!");
      }
    },
    init: function(){
      //alert("into  PushService init function!");
      var me = this;
      if (ionic.Platform.isIOS()){
        me.pushType = 1;
        me.platForm = 'iOS';
    ...
    onReady: function(deviceId){
      ...
      console.log("pushNotification init success:" + deviceId);
    ...
    register: function(){
      ...
      if (me.isReady){
        ...
        $http({
          ...
          //alert("PushServiceAddUserPush.success");
          console.log("PushServiceAddUserPush.success");
        });
      }
    },
    unregister: function(){
    ...
    gotoUrl: function(event){
      ...
      //var me = this;
      //alert("gotoUrl: " + event.alert);
      //me.state.go(url);
      //me.state.go("tab.home.toread");  
        $state.go(url, {"immediateRefresh": true});
    }
  };
  /*
